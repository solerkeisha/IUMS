...
def add_missing_columns():
    """Add missing columns to existing tables including due_date"""
    conn = get_connection()
    if not conn:
        return False
        ...
        if 'accounts' in old_data:
            for username, account_data in old_data['accounts'].items():
                ...
                cursor.execute('''
                    ...
                    (username, password, role, debt_limit, personal_info, created_date)
                    VALUES (?, ?, ?, ?, ?, ?)
                ''', (
                    username,
...
def check_database_health():
    ...
    try:
        ...
        if missing_columns:
            conn.close()
            add_missing_columns()
            return True, f"Added missing columns: {', '.join(missing_columns)}"
