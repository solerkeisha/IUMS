...
from database import get_connection, init_database, migrate_from_json, add_missing_columns, check_database_health
from email_utils import email_service  # Add this import
...
def create_account(username, password, role, personal_info=None):
    """Create new account"""
    ...
    try:
        cursor.execute('''
            INSERT INTO accounts (username, password, role, debt_limit, personal_info, created_date)
            VALUES (?, ?, ?, ?, ?, ?)
        ''', (username, password, role, debt_limit, personal_info_json, get_current_datetime()))
...
def create_pending_transaction_with_due_date(customer, transaction_type, description, amount, created_by=None, interest_rate=0, due_date=None):
    ...
    try:
        ...
        transaction = {
            "id": transaction_id,  # Make sure this is included
            "customer": customer,
        ...
        print(f"✅ Transaction created with ID: {transaction_id}")  # Debug log
...
def get_customer_list():
    """Get list of customer usernames"""
    try:
...
try:
    ...
    class DummyEmailService:
        ...
        def send_otp_email(self, *args, **kwargs):
            print("❌ Email service not available")
            return False
        
        def send_due_date_reminder(self, *args, **kwargs):
            print("❌ Email service not available")
            return False
...
def test_email_configuration():
    ...
    try:
        ...
        if email_service.is_configured:
            # Test connection if the method exists
            if hasattr(email_service, 'test_connection'):
                ...
                if success:
                    ...
                    print("❌ Email service configuration error")
                    return False
            else:
                print("✅ Email service is configured (connection test not available)")
                return True
        else:
